    ...
    val patchList = designDB.members.view.collect {
      ...
      case waitMember @ Wait(triggerRef = DFRef(cyclesVal @ DFDecimal.Val(DFUInt(_)))) =>
        val replaceWithWhile = cyclesVal match
          ...
          case _                                                     => true
        if (replaceWithWhile)
          import dfhdl.core.{DFUInt, asValOf, dfType}
          val iterType = cyclesVal.asValOf[DFUInt[Int]].dfType
          val regDsn = new MetaDesign(
            waitMember.getOwnerProcessBlock,
            Patch.Add.Config.Before,
            dfhdl.core.DomainType.RT(dfhdl.core.RTDomainCfg.Derived)
          ):
            val waitCnt = iterType.<>(VAR.REG)(using dfc.setName("waitCnt"))
          val dsn = new MetaDesign(
            waitMember,
            Patch.Add.Config.ReplaceWithLast(Patch.Replace.Config.FullReplacement),
            dfhdl.core.DomainType.RT(dfhdl.core.RTDomainCfg.Derived)
          ):
            import regDsn.waitCnt
            ...
            waitCnt.din := dfhdl.core.DFVal.Const(iterType, Some(BigInt(0)))
